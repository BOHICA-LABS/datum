...
| **Dolt refuses to merge unrelated lineages: `no common ancestor`.** Independently `dolt init`-ed databases can never be merged, only replayed. | Every writer must be a CLONE of the artifact branch. Also the hard proof that fragmenting into per-instance refs is a ONE-WAY DOOR, not a tradeoff. | `test_ci_aggregator` |
| **`git push <url> :ref` needs a git REPOSITORY.** In a CI job with no `actions/checkout`, cwd = a Dolt repo (`.dolt/`), every deletion failed in **0.7 ms with no network**. | Delete refs via the API (`gh api -X DELETE repos/O/R/git/<ref>`) when there is no git repo. And ASSERT the result — see §2. | `test_ci_aggregator` |
| **`on: push` never fires for `refs/dolt/*`** (only `refs/heads/*` and `refs/tags/*`; there is no `refs:` key). | Publishing a staging ref is invisible to CI. Use `repository_dispatch`/`workflow_dispatch`/`schedule`. | verified in docs |
| **GitHub keeps at most ONE run pending per `concurrency` group and CANCELS the rest.** | A writer publishing mid-run can lose its trigger. Re-enumerate at the end of a run and re-dispatch on progress; `queue: max` and a cron sweep are layers 2 and 3. Confirmed live — a run shows conclusion `cancelled` while the work still landed. | `test_ci_aggregator` C3 |
| **GitHub Actions runs `bash -e`**, so `VAR=$(cmd)` aborts the step before your own error handling runs — the step dies with NO output. | Set the shell explicitly (`shell: bash -uxo pipefail {0}`) when the script manages its own control flow. | `test_ci_aggregator` |
| **An invalid workflow YAML fails almost silently:** dispatches return rc=0, no run is created, and the tell is that `gh api …/workflows` reports the workflow `name` as its FILE PATH. | Lint the YAML locally before pushing. A block-scalar continuation dropped to column 1 caused it. | `test_ci_aggregator` |
| **`GITHUB_TOKEN` CAN create, update and delete `refs/dolt/*`** with `permissions: contents: write`. | CI can be the aggregator without a PAT. Pushes made with it do not retrigger workflows, which prevents dispatch loops for free. | `test_ci_aggregator` C1b |
...
| **A second process opening the same directory succeeds as READ-ONLY**, then fails at write time with `cannot update manifest: database is read only`. It does not refuse to open. | Single-writer per directory. A long-lived `datum` that opens early and writes later can be silently read-only for its whole life — `doctor` must check **writability**, not openability. |
| **The `dolt` CLI and an embedded process cannot write one directory concurrently** — the CLI gets the same read-only manifest error. | A migration cannot run `datum` embedded and `dolt sql` side by side without one mutex covering both. |
...
| Remote ops **do** work in-process: `DOLT_PUSH` to GitHub succeeded from the driver. | An embedded `datum` needs no `dolt` binary at all — that is the strongest argument for it. |
