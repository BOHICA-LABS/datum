...
| **ALL Dolt branches live inside ONE git ref.** Pushing a new Dolt branch creates no new git ref; it rewrites the single data ref. | Push contention is **global** across instances, not per-branch. A fresh unrelated database pushing to an existing data ref is a non-fast-forward. Use `--ref` per instance to decouple. |
