---
title: STORYBOARD-METHOD-ASSESSMENT — what the persona-storyboard process contributes to datum, and what datum should retire
date: 2026-08-02
source_read: ~/Dev/multi-repo/.factory-project/storyboard/PERSONA-STORYBOARD-PROCESS.md (1,951 lines, read in full)
purpose: decide which of the runbook's methods transfer into our tooling, which are superseded by datum v1, and what it contributes as a validation register
status: ASSESSMENT — no method adopted yet; 7 transfer candidates named, 3 of them load-bearing on L7
---

# The persona-storyboard process, assessed against `datum` v1
...
recurrences of exactly the defect classes `datum` v1 claims to make unrepresentable** — produced by a
...
### The lineages, mapped to `datum` mechanisms

| Storyboard lineage (as the corpus itself numbers it) | Recurrences | `datum` v1 mechanism |
|---|---|---|
...
burden, striking its own false scalar in place) is better than most of what `datum`'s own corpora show.

---

## 2. What genuinely TRANSFERS — 7 candidates, 3 of them load-bearing

These are things the runbook has that `datum`'s ~6,300 lines of design **do not**, judged against the
...
The runbook independently invented `datum`'s caller-asserted-vs-verified distinction and pushed it
**one tier further**:
...
`datum`'s invariant 22 is **binary** (evidence exists iff `datum` produced it); V-K adds *caller-asserted*.
The runbook's model is **richer in the exact place V-K is thinnest**, and it supplies the piece `datum`
...
**LOAD-BEARING. Becomes a hard rule for every `datum` gate.**
...
but **not to its own gates**. `datum` has the adjacent vacuity guard (a gate must not pass on an empty
...
input the gate provably rejects — and `datum gate exec` refuses to register a gate with no witness. This
...
Translated to a CLI/MCP tool, this is the L7 completeness gate `datum` needs and does not have:

> **For every one of the ~800 generated ops, enumerate and prove reachable every exit code —
> `0` success · `1` gate failed · `2` `datum` failed · `3` `DENIED-ASYMMETRY` · `4` `propose` — with a
> golden-file capture of the exact structured output for each, captured deterministically.**

`datum`'s design already has the exit-code vocabulary and the standing rule *never collapse exit 1 and
...
Also transfers: the **content-extreme state** (longest label, densest table). For `datum` that is the
...
### T-4 — The AGENTIC-UX-RUBRIC (A1–A4) as `datum`'s agent-facing error-design rubric
...
| | rubric dimension | `datum` L7 obligation |
|---|---|---|
...
| **A3** | human-in-the-loop before impactful action — approve/reject/**refine** | `datum propose field` (exit 4) **is** A3; so is refusing conflicts rather than merging (invariant 21) |
...
`datum` has `derivation_stage` (shadow→proven→retired) and the migration ladder
...
`datum` has projections and invariant 19 (every aggregate declares a scope predicate). What it lacks is
...
a **declared scope predicate** or derivation silently changes the document's meaning — and `datum` is
...
| the 18 hand-widened sweep-population criteria | These **are** the defect being documented. `datum`'s answer is invariant 17, not criterion 19. |
| breakpoint matrices · DTCG tokens · a11y annotation layers · Nielsen heuristic eval · WCAG reflow | UI-specific; `datum` has no UI. (The *determinism* discipline behind Stage 7 does transfer — see T-3.) |
| human personas, Persona Spectrum, anti-personas, emotion curves, service blueprints | Presuppose human users. See §4 — `datum`'s cast is agents, and the runbook's own agent-persona class is its weakest-grounded. |
...
**Under V-K, `datum`'s consumer is an LLM harness, not a human.** So Stage 1's cast is almost entirely
...
| Stage | Applies to `datum`? | Form it takes |
|---|---|---|
| 1 Personas | **partly** | agent personas (Claude Code, Codex, CI) + **anti-personas** are unusually apt: a confused-agent retry storm, a role-token forger, an agent that fabricates evidence. Anti-personas trace to abuse vectors and are **exempt from the orphan check** — matches `datum`'s wall/asymmetry model. |
...
| 7 Evidence | **yes, as discipline** | determinism: pinned versions, mocked timestamps, byte-identical re-runs. `datum` already holds this bar (no network, no `dolt` binary, 124 tests in 7 s). |
...
`datum` emits, and it is nearly free.
