...
|---|---|---|
| **V-A** | **`datum` is the source of truth. Markdown is a rendered view.** | Authoring goes through `datum` against a closed schema. Committed markdown is a generated review surface + offline backup, gated byte-exact. Every skill/agent that writes `.factory/**` is rewritten to call `datum`. |
| **V-B** | **v1 scope is the whole operational substrate** — store, gates, engine, scheduler, PR/CI join, cost, attestation. | `datum` replaces the markdown-plus-hooks machinery rather than sitting beside it. The 62 hooks, 5 bash helpers and 7 hand-maintained indexes are retirement targets, not integration targets. |
...
- **Evidence exists only if `datum` produced it.** `datum gate exec` is the *sole* writer of the evidence
...
- **No write escape hatch** — four cases, four answers: prose goes through D-A, a missing field goes
  through `datum propose field` (exit 4), some requests are impossible by construction, and read-only
  curiosity gets `sql --read-only`. Never raw write SQL.
- **`datum render` is four renderers keyed on `(authority, shape)`** — ledger / authored / derived / blob.
...
⚠ **`datum render` is blocked on schemas, not on an engine.** **All 22 derived types declare zero
...
| Defect class found by the review | Under `datum` v1 |
|---|---|
...
| ~28 unregistered artifact homes; 225 unmatched files; `planning` vs `plans`; case drift | **Impossible** — agents never name a path; `datum` computes it. |
| ~14 finding ID families; a format hook validating one of them | **Impossible** — one namespace, minted by `datum`. |
...
**The residue that `datum` cannot make impossible** and must therefore *detect* well: semantic
...
(`datum bc set-postcondition`, `datum finding add`, `datum gate record`), never SQL and never file paths. That
...
shadow  →  dual-write  →  authoritative  →  markdown retired
   |            |               |                  |
 datum shadow   datum writes      datum is the         the .md files and
 reports     + .md still    only writer;      their hooks are
 disagreement  authored     .md is rendered   deleted
```

`datum shadow` already implements stage 1 and reports 658 disagreements — that is precisely the evidence
...
pr-reviewer, spec-reviewer, session-reviewer — to genuinely different model families, with `datum`
...
**What v1 must build now so this is a config change later, not a redesign:** `datum` records the model
...
## 5c. V-J — `datum` IS RUN WITH AN AI. The AI is the parser for messy input.

**Decision (2026-08-02):** `datum` is not a tool a human drives against tidy input. It runs **with an AI
...
2. **`unevaluable = block` remains, for gates.** An AI must never be the thing that decides a gate
   ...
   evidence exists only if `datum` produced it.
3. **No AI write path into the store that bypasses validation.** The AI's output is a *proposal* that
   goes through the same ops and the same ladder. L3–L4's `datum propose field` (exit 4) is exactly the
...
## 5d. V-K — SETTLED: the AI runs OUTSIDE. `datum` is a TOOL for an LLM harness, via CLI or MCP.

**Decision (2026-08-02), closing V-J's open question:** `datum` contains **no LLM client, no API keys, no
model configuration and no provider network calls.** It is a **tool designed to be used BY an LLM
harness** — Claude Code, Codex, or any other — exposed as a **CLI and an MCP server**. `datum` is not an
agent and never becomes one.

### The control flow is INVERTED from what "AI-assisted ingest" suggests

`datum` never calls an AI for help. **`datum` emits work; the agent interprets; `datum` records the answer.**

```
   harness/agent  ──calls──▶  datum           "what still needs classifying?"
   datum             ──emits──▶  work unit    the file, its content, its candidates
   agent          ──reads──▶  interprets   (this is the part datum cannot do)
   agent          ──calls──▶  datum           records the classification + evidence
   datum             ──validates, versions, audits, and REPLAYS thereafter
```
...
- **The harness supplies identity.** The role token (F16), session/trace id, and the model identity V-I
  ...
  from the harness injecting it — a `PreToolUse`-style hook — because `datum` cannot distinguish agents
...
1. **`datum` cannot VERIFY a model claim — only record what the caller asserts.** So an attestation row
...
2. **Tool-call ergonomics become a hard design constraint, and this one is load-bearing.** A harness
   ...
   be **chunked, resumable, progress-reporting and batchable**, with `datum` emitting the *next* unit
...
response envelope; `datum migrate verify` **refuses on a moved pin**; baselines and ratchets *are* diffs
...
`datum/graphsql_probe_test.go` (`TestGraphInSQL`). GMS — pure Go, **no `dolt` binary** — accepts
**recursive CTEs**:
...
⚠ **This CORRECTS an overclaim in `datum`'s own help text**, which said *"algorithms SQL cannot do."* That
...
The real risk is defending *"Dolt"* because the repo is named `datum (formerly dolt-artifact-spike)`. So **L1 declares
...
- **Lossless — and the bar is split in two, because a single 100% claim is unachievable.**
  ...
  than no bar, so:
  - **Round trip (invariant 15) must be 100%** of what `datum` holds. An untyped file is held as a
...
- **Reversible:** `datum render` reproduces a committed markdown tree that re-imports byte-identically.
