...
# Master workflow inventory — `fa`

## 1. The lifecycle-stage spine (Stage 2 step 1)

Seven ordered stages reflecting `fa`'s actual usage arc. Chosen to hold every workflow without forcing
...
| return | **caller ignores exit 6** | ⚠ **undetectable by `fa`** — see G-7 | ⚠ NEW |

⚠ **G-7 (new).** `fa` cannot detect a caller that stops following `next`. A migration left half-done
...
adjudicating a `fa propose field` proposal (WF-025) — see **G-1**. Per the runbook, an accountable RACI
...
| **G-7** | ⭐ NEW — `fa` cannot detect an **abandoned** `CONTINUATION` | §3 extension brainstorm | L7 §3; `doctor` finding past a staleness window |
