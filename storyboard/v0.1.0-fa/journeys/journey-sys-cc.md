...
> expect `fa` to hand me work, not to ask me to guess at its internals.
...
- **Decisions/branches:** none for me. This is the one stage where `fa` needs nothing from me.
...
- **Op:** `fa next --kind classify --limit 20` → I interpret → `fa record classify --cursor <c> …`
- **Agentic touchpoint:** ⭐ **this is the one place `fa` genuinely needs an LLM.** **A1** `fa` states
...
- **Op:** `fa bc set-field --key BC-5.39.009 --field lifecycle_status --value active --idem <key>`
...
- **Op:** `fa migrate step --project rivetry --cohort B --limit 200 [--cursor …]`
...
- **Handoffs:** ⚠ **G-7 — if I simply stop, nothing notices.** `fa` cannot distinguish a cell I
...
| a wrong classification is durable once replayed | S2 classify | needs a **re-open** path: `fa reclassify` must exist, or a recorded decision is permanent. **NEW → G-8** |
