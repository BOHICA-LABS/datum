...
# Anti-persona roster — `fa`

This is the stage of the runbook that transferred **best**. `fa`'s whole design is a set of perimeters,
...
The runbook's trigger conditions all fire: `fa` touches **tenant isolation** (per-project stores),
...
**Defences:** `fa gate exec` is the **sole** writer of the evidence table and no flag accepts evidence
...
**Status:** fully defended in design. ⚠ The residue is stated honestly: `fa` **cannot verify** an
...
**Status:** ⚠ **two open gates**, both already recorded: `fa perimeter explain` must be **proven** a
...
**Consequence:** measured, this session, from two instances in `fa`'s own code: **80 VPs → 0 rows** with
...
| `fa` surface handling something sensitive | anti-persona | or risk-accepted rationale |
|---|---|---|
| tenant isolation (per-project stores) | **AP-4** | — |
| gate verdicts / evidence | **AP-2** | — |
| information-asymmetry perimeter | **AP-3** | — |
| role tokens / identity | **AP-2**, **AP-3** | ⚠ `fa` cannot verify identity at all — accepted and declared (L7-K), not defended |
