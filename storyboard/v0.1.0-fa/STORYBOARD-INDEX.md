> **Type:** storyboard exploration corpus, version `v0.1.0-fa`. **Status:** first pass, Stages 1–3
...
> full, 1,951 lines), applied to `fa` as the product. **Legend:** CANONICAL stores hold each artifact
...
consumer of `fa` is an **LLM harness**, not a human, so Stage 1's cast is almost entirely its Step-3d
...
| 1 Personas | **yes, reframed** | agent personas + **anti-personas** (unusually apt for `fa`) → `personas/` |
...
Adapted from the runbook, mapped to `fa`'s own ladders. A cell advances in this order only; a
