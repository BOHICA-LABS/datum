...
| **the LLM provider** | — | `deliberately-out-of-scope` — V-K: `datum` contains no LLM client, no keys, no provider calls. It is NOT in `datum`'s ecosystem at all, and that is the decision, not an omission |
...
`datum propose field` (exit 4) proposal?* The op files a proposal and writes nothing; L3–L4 names a
...
**Auth posture:** identity is **caller-asserted, never verified** — `datum` cannot distinguish agents
...
**Known failure mode:** a design that quietly assumes a Claude-specific hook. `datum` has exactly one
...
**Hard boundaries:** no approval; no interactive prompt; `datum gate exec` is the same op with the same
evidence typing.
...
makes markdown a view, so the human's authoring surface is `datum`, same as an agent's).
...
stated reason: `datum`'s human surface is a terminal and a markdown render, and no accessibility work is
