...
1. ⚠⚠ **The `📸` column is EMPTY for every persona.** Nothing in `datum` has op × exit-code golden-file
...
5. **HUM-OP carries the most `⬜` (6).** The human-facing surface (render, round-trip proof, cost,
   worktree hygiene, the cube itself, proposal adjudication) is the least built — consistent with `datum`
