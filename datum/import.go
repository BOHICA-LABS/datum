...
// `datum import` — build the shadow store from the markdown corpus.
//
...
// throwaway. If `datum` is wrong here, a CI job is wrong; the corpus is untouched.
//
...
// idempotent; the migration ledger records the version so `datum doctor` can tell a
...
func ensureSchema(ctx context.Context, s *Store) error {
	...
	if _, err := s.Exec(ctx,
		...
		schemaVersion, fmt.Sprintf("datum %s zone %s", faVersion, s.Zone)); err != nil {
