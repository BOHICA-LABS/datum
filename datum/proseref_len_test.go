...
//
// `datum import ~/Dev/prism/.factory` hard-aborts on `Error 1105: string '...' is too
...
//
//   DATUM_LEN_CORPORA=~/Dev/vsdd-factory/.factory,~/Dev/prism/.factory,~/Dev/rivetry/.factory \
...
func TestProseTargetLengths(t *testing.T) {
	spec := os.Getenv("DATUM_LEN_CORPORA")
	if spec == "" {
		t.Skip("set DATUM_LEN_CORPORA to a comma-separated list of .factory roots")
	}
