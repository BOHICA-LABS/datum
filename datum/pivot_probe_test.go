...
func TestPivotCost(t *testing.T) {
	corpus := os.Getenv("DATUM_PIVOT_CORPUS")
	if corpus == "" {
		t.Skip("set DATUM_PIVOT_CORPUS to a .factory corpus to run the pivot measurement")
	}
...
func TestPivotStorage(t *testing.T) {
	corpus := os.Getenv("DATUM_PIVOT_CORPUS")
	if corpus == "" {
		t.Skip("set DATUM_PIVOT_CORPUS to a .factory corpus to run the storage measurement")
	}
