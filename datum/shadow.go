...
// `datum shadow` — STORY 7, the SHADOW stage of `generate -> prove equal -> retire`.
//
...
//	proven   they agree; `datum render` writes it; the authored one is still diffed
...
//
// Scope note, so the two gates are not confused. `datum validate` already compares the COUNTS
	...
	for _, spec := range shadowSpecs {
		...
		if ts.DerivationStage != "shadow" {
			find("shadow.stage-not-shadow", spec.Type,
				fmt.Sprintf("derivation_stage is %q; `datum shadow` implements the `shadow` stage only "+
