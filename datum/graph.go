...
// that does not apply to `datum` today only because the store is rebuilt from files per run.
//
// gonum, not petgraph-equivalent-of-the-week, for one hard reason: it is PURE GO. `datum`
...
func BuildGraph(ctx context.Context, s *Store, reg *RegistryBundle, asOf string) (*Projection, error) {
	...
	// resolve is datum FAILING (exit 2), not a graph with nothing in it.
	if asOf != "" {
