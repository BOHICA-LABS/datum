...
// The schema, as statements rather than a .sql file: `datum` is one binary and must
...
var openDDL = []string{
	...
	// and it exists so `datum validate` can catch the claim disagreeing with reality.
	...
	// cell), so `datum doctor` and CI can say WHICH corpus state a store reflects.
	//
...
// `datum validate --cross-zone` buys that guarantee back in the tool (see D2:
