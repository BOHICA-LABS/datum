...
const usage = `datum — the sole interface to factory artifacts (phase 1: read-only shadow)

usage: datum <command> [flags]
...
common flags:
  --db <dir>                 store root (default .datum-db, or $DATUM_DB)
...
func main() {
	...
	case "version":
		fmt.Printf("datum %s (schema v%d, embedded dolthub/driver/v2)\n", faVersion, schemaVersion)
	case "-h", "--help", "help":
		fmt.Print(usage)
	default:
...
func cmdInit(ctx context.Context, args []string) error {
	...
	for _, zone := range []string{ZoneOpen, ZoneWalled} {
		...
		changed, err := s.DoltCommit(ctx, "schema: initialise zone "+zone+" (datum "+faVersion+")")
		if err != nil {
			s.Close()
			return err
		}
...
func cmdImport(ctx context.Context, args []string) error {
	...
	_ = fs.Parse(args)
	if fs.NArg() != 1 {
		return fmt.Errorf("usage: datum import <corpus-path>")
	}
...
func cmdValidate(ctx context.Context, args []string) error {
	...
	// Exit 1 = the gate failed. Exit 2 (elsewhere) = datum itself failed. Never swallow
...
func cmdShadow(ctx context.Context, args []string) error {
	...
	_ = fs.Parse(args)
	if fs.NArg() != 1 {
		return fmt.Errorf("usage: datum shadow <corpus-path> [--db <dir>] [--json <path>]")
	}
	...
	// and `datum` breaking is exit 2 elsewhere. Never collapse the two.
	if *baselinePath != "" {
		b, err := LoadBaseline(*baselinePath)
		if err != nil {
			return fmt.Errorf("baseline: %w", err)
...
func cmdBaseline(ctx context.Context, args []string) error {
	if len(args) == 0 || args[0] != "write" {
		return fmt.Errorf("usage: datum baseline write [--out <path>] [--db <dir>] [--corpus <path>]")
	}
...
// gitRef records WHICH corpus state a baseline was taken from. Best effort: `datum`
...
func cmdAggregate(ctx context.Context, args []string) error {
	if len(args) == 0 || args[0] != "plan" {
		...
		return fmt.Errorf("`datum aggregate` needs a remote, which phase 1 excludes (DECISIONS D3).\n" +
			"The quarantine policy it will use IS implemented and tested — inspect it with:\n" +
			"  datum aggregate plan --state <state.json> --refs <ref,ref,...> --run <n>")
	}
