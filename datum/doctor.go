...
func checkSchema(ctx context.Context, s *Store) Check {
	...
	if err != nil {
		c.Detail = "no schema_migrations table — run `datum init`"
		return c
	}
	switch {
	case v == 0:
		c.Detail = "store has no schema — run `datum init`"
	case v < schemaVersion:
		c.Detail = fmt.Sprintf("store at schema v%d, binary expects v%d — run `datum init`", v, schemaVersion)
	case v > schemaVersion:
		c.Detail = fmt.Sprintf("store at schema v%d is NEWER than this binary (v%d) — upgrade datum", v, schemaVersion)
	default:
		c.OK = true
...
func checkIdentity(ctx context.Context, s *Store) Check {
	...
	// driver takes identity from the DSN, so `datum` always has one — report which,
	// because a store full of commits by "datum <datum@local>" on a shared machine is a
	...
	if commitName() == "datum" && commitEmail() == "datum@local" {
		c.Detail += " (default — set DATUM_COMMIT_NAME/DATUM_COMMIT_EMAIL for attributable history)"
	}
	return c
}
...
func checkImported(ctx context.Context, s *Store) Check {
	...
	if n == 0 {
		...
		c.Detail = "no records — run `datum import <corpus>`"
		return c
	}
	c.OK = true
	if s.Zone == ZoneWalled {
