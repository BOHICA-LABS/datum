...
a `dolt` binary. Markdown remains the single source of truth. If `datum` is wrong, a
CI job is wrong.
...
```sh
CGO_ENABLED=1 go build -tags gms_pure_go -o datum .
```
...
```sh
datum init   --db .datum-db                      # create both zone directories
datum import --db .datum-db ~/path/.factory      # shadow the corpus (idempotent)
datum validate --db .datum-db --baseline baseline.json
datum doctor --db .datum-db
datum count  --db .datum-db --by-subsystem       # counts, derived — never stored
datum baseline write --db .datum-db --out baseline.json --corpus ~/path
datum waves  --db .datum-db                      # wave schedule, derived from depends_on
datum graph build|metrics|dot|diff --db .datum-db
datum shadow --db .datum-db ~/path/.factory      # STORY 7: derived indexes, generated ALONGSIDE
```

`datum shadow` is story 7's **shadow** stage of `generate -> prove equal -> retire`. It
...
Exit codes: **0** the gate passed · **1** the gate failed · **2** `datum` itself
...
| | |
|---|---|
| `datum import` | **1.1–1.4 s** (scan + one transaction + Dolt commit) |
| `datum validate` | **0.15 s** |
...
The port is verified against `poc/datum.py` + `poc/graph_import.py`, not merely
...
subset of `datum`'s 1,509 (see the correction below).
...
`datum` requires the value to actually *open* an inline list before treating following
...
agrees with disk. `datum` therefore treats enumeration and counting as two separate
...
**`datum doctor` checks WRITABILITY, not openability.** A second opener of a Dolt
...
**`datum aggregate` quarantines stuck staging refs.** A conflicted staging ref is
...
the policy with `datum aggregate plan --refs ... --state ... --run N`; `datum aggregate`
...
| `quarantine.go` | the `datum aggregate` staging-ref policy |
| `workflows/datum-validate.yml` | the CI gate (deploy to `.github/workflows/` in vsdd-factory) |
...
Phase 1 is `import` + `validate` only. `datum get/trace/coverage/history/asof/diff`,
