...
func TestShadowSpecsAreDeclaredAndAtShadowStage(t *testing.T) {
	...
	for _, spec := range shadowSpecs {
		...
		if ts.DerivationStage != "shadow" {
			t.Errorf("spec %q is at derivation_stage %q; `fa shadow` implements the shadow stage only",
...
// TestShadowRejectsBadCorpusRoot: a wrong path must be fa FAILING, not a clean gate.
