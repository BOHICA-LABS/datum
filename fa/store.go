...
// The store layer. `fa` talks to Dolt through the EMBEDDED dolthub/driver/v2 —
...
//      So `fa doctor` must probe WRITABILITY, not openability — see doctor.go.
...
func Open(ctx context.Context, root, zone string, create bool) (*Store, error) {
	...
	if create {
		...
		return nil, fmt.Errorf("zone %s not initialised at %s (run `fa init`)", zone, dir)
	}
...
func commitName() string {
	if v := os.Getenv("FA_COMMIT_NAME"); v != "" {
		return v
	}
	return "fa"
}
...
func commitEmail() string {
	if v := os.Getenv("FA_COMMIT_EMAIL"); v != "" {
		return v
	}
	return "fa@local"
}
